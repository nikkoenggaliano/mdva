...
router.post('/password', async (req, res) => {
  try {
    ...
    if (!old_password || !new_password || !confirm_password) return res.status(400).json({ message: 'All fields are required' });
    ...
    if (user.password !== md5Hash(old_password)) return res.status(400).json({ message: 'Old password is incorrect' });
