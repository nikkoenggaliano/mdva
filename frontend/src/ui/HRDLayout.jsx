...
import { AppBar, Toolbar, IconButton, Typography, Drawer, List, ListItem, ListItemText, Box, Container, useTheme, Tooltip, useMediaQuery, ListItemIcon, Menu, MenuItem } from '@mui/material'
...
export default function HRDLayout() {
  ...
  const menu = [
    ...
    { to: `${base}/inventory-requests`, label: 'Inventory Requests', icon: <ListAltIcon /> },
      ...
      <AppBar position="fixed" color="transparent" enableColorOnDark sx={{ backdropFilter: 'blur(8px)', backgroundColor: theme.palette.mode === 'dark' ? 'rgba(10,16,28,0.7)' : 'rgba(255,255,255,0.7)', borderBottom: '1px solid', borderColor: theme.palette.mode === 'dark' ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.06)' }}>
        <Toolbar>
          ...
          <Menu anchorEl={anchorEl} open={menuOpen} onClose={() => setAnchorEl(null)}>
            <MenuItem onClick={() => { setAnchorEl(null); navigate('/hrd/profile') }}>My Profile</MenuItem>
